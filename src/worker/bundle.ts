...
export function renderBundle(b: BundleData): string {
  ...
  const imageUrls = [
    ...new Set([
      ...extractImageUrls(action.description, b.baseUrl),
...
function renderArcActionSection(b: ArcActionData, baseUrl: string): string[] {
  ...
  const imageUrls = [
    ...new Set([
      ...extractImageUrls(action.description, baseUrl),
...
export function renderArcBundle(b: ArcBundleData): string {
  ...
  // key, not a branch/PR per action.
  out.push("---", "", "## How to deliver this work", "");
  out.push(
    `You're taking on the whole **${arc.name}** arc — the ${list.length} open action${
      list.length === 1 ? "" : "s"
    } above${keys.length ? ` (${keys.join(", ")})` : ""}. Drive them as one coordinated change that lands in a **single pull request**.`,
    "",
    `1. **Plan the split.** Read every action above and decide a sensible division of labor. Watch for actions that touch the same files or depend on each other — sequence or group those so sub-agents don't fight over the same code.`,
  ...
  out.push(
    ...
    "1. **Analyze** — `git status` and `git diff` (incl. `--cached`) to see exactly what changed.",
    "2. **Security check** — scan the diff for secrets, API keys, passwords, tokens, or PII. If you find any, **STOP**, do not commit, and flag it.",
    "3. **Plan** — one commit per logical unit of work; keep unrelated changes in separate commits, and name the action key the commit advances.",
    "4. **Commit** — use [Conventional Commits](https://www.conventionalcommits.org/): `type(scope): KEY subject` (types: feat, fix, docs, style, refactor, perf, test, build, ci, chore, revert), where `KEY` is the action that commit advances. Subject in imperative mood, no trailing period; the body explains *why* (context), *what* (the change), and any side effects. Do **not** add `Co-Authored-By` or any AI/Claude attribution.",
    "5. **Verify** — `git status` and `git log` to confirm the history is clean and complete.",
