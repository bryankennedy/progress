...
// PROG-40: the Done column is capped to the most-recently-completed actions so it
...
async function markDone(page: Page, n: number): Promise<number> {
  ...
  const subset = ws.actions.slice(0, n) as { id: string; status: string }[];
  touched = subset.map((i) => ({ id: i.id, status: i.status }));
  // Sequential so completedAt timestamps differ (the API sets it to "now").
  for (const { id } of touched) {
    await page.request.patch(`/api/actions/${id}`, { data: { status: "done" } });
  }
  const after = await (await page.request.get("/api/snapshot")).json();
  return after.actions.filter((i: { status: string }) => i.status === "done").length;
}
...
test.afterEach(async ({ page }) => {
  for (const { id, status } of touched) {
    await page.request.patch(`/api/actions/${id}`, { data: { status } });
  }
  touched = [];
});
...
test("the Done column shows at most the 10 most recent, noting the total (PROG-40)", async ({
  ...
  await expect(done.locator("[data-action-id]")).toHaveCount(10); // exactly 10 cards rendered
});
