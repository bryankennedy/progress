...
function chunk(t, d) {
  ...
  cc.writeUInt32BE(crc32(Buffer.concat([tt, d])), 0);
  return Buffer.concat([l, tt, d, cc]);
}
const hex = (h) => {
  h = h.replace("#", "");
  return [0, 2, 4].map((i) => parseInt(h.slice(i, i + 2), 16));
};
function bez(P0, C1, C2, P3, n = 56) {
  const p = [];
  for (let i = 0; i <= n; i++) {
    const t = i / n,
      u = 1 - t,
...
function sdRR(px, py, half, rr) {
  const qx = Math.abs(px - half) - (half - rr),
    qy = Math.abs(py - half) - (half - rr);
...
const BARS = [
  ...
  const mx = (xL + xR) / 2,
    C1 = [mx - (mx - xL) * 0.4, cy - BOW],
    C2 = [mx + (xR - mx) * 0.4, cy - BOW];
  ...
  return {
    ...
    col: hex(col),
    colHex: col,
    bb: [minx - RAD, miny - RAD, maxx + RAD, maxy + RAD],
...
function renderPng(size, mode, markScale = 1) {
  const SS = 4,
    ...
    bg = hex(CREAM);
  const stride = size * 4,
    raw = Buffer.alloc((stride + 1) * size);
      ...
      for (let sy = 0; sy < SS; sy++)
        for (let sx = 0; sx < SS; sx++) {
          ...
          if (onIcon) {
            ...
            const ux = 50 + (px / scale - 50) / markScale,
              uy = 50 + (py / scale - 50) / markScale;
            for (const bar of BARS) {
              const [a, c, d, e] = bar.bb;
              if (ux < a || ux > d || uy < c || uy > e) continue;
              if (distPoly(ux, uy, bar.pts) <= RAD) col = bar.col;
            }
            r += col[0];
            g += col[1];
            b += col[2];
...
function svg(rounded, markScale = 1) {
  ...
  const rect = rounded
    ? `  <rect width="100" height="100" rx="22.5" fill="${CREAM}"></rect>`
    : `  <rect width="100" height="100" fill="${CREAM}"></rect>`;
  // scale the mark about the centre; the group transform scales stroke-width too.
  const xf =
    markScale === 1 ? "" : ` transform="translate(50 50) scale(${markScale}) translate(-50 -50)"`;
