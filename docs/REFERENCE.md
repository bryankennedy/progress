...
| `GET /api/actions/:key/bundle` | Looked up by **key** (alias-aware), not id. Returns `text/markdown` — a deterministic context "work order": action fields + tags (unset fields omitted — PROG-112), lineage with descriptions (focus incl. optional `gitUrl` → arc, where the arc description carries the "why", plus the **Step ancestor chain** and a **Steps** list of child actions — PROG-112, closing the PROG-106 gap), comments, an **Images** list (absolute URLs of every image referenced in the description/comments, so a bearer-authed agent can fetch them — PROG-42), linked PRs/commits — empty collections render no section at all — then a stable report-back preamble — **branch off fresh `origin/main`, never another feature branch, and PR with `--base main`** (PROG-95), branch/key auto-linking + status flow, plus a **Committing & PRs** block that embeds a local, key-aware copy of the owner's smart-commit conventions (secret-scan, logical conventional commits `type(scope): KEY subject`, run the repo's checks before pushing, no AI attribution on commits **or the PR body**) so a handed-off agent commits to the owner's rules (PROG-62/PROG-112). A focus with no `gitUrl` gets a short comment-and-status report-back instead of the git machinery. A retired key resolves and renders the current canonical key. 400 malformed key, 404 unknown. Rendered by `src/worker/bundle.ts` (`renderBundle`); shared foundation for the agent surfaces (SPEC §11.1, D33). |
...
- **CLI** — `bin/progress.ts` (`progress work <KEY>`): fetches the bundle with
  ...
  `--model <name>` (passed through as `claude --model`, so the launcher — not
  the prompt — picks the model; PROG-112). Registration: SETUP §7.
...
- **Outline (`/outline`, PROG-124)** — a Workflowy-style outliner for fast
  ...
  vice-versa. A drop whose neighbours carry **equal** ranks (racing creates
