...
**Always append at the very bottom; merges keep both.** `.gitattributes` marks
this file `merge=union`, so when two branches each append an entry at EOF, git
keeps **both** on merge/rebase instead of stopping for a conflict (the
resolution was always "keep both" anyway — see `DECISIONS-union` below). This
only works because the log is **append-only**: union silently merges edited
lines without a conflict marker, so never rewrite a settled entry on a feature
branch — supersede it with a new entry instead. After a union merge, glance at
the joined tail and add the blank line between two back-to-back entries if it's
missing.
