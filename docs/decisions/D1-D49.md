# Decisions D1–D49 (frozen legacy log)

The numbered era of the decision log, moved verbatim from `docs/DECISIONS.md`
in the PROG-91 split. Cite these as `D<n>`, exactly as before. This file is
**frozen**: it only ever changes if an entry is superseded — and supersession
happens by writing a *new* entry in the superseding issue's own file, never by
editing this one.
